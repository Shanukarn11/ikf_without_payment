...
def teamlist(request, id):
    ...
    no_of_teams = toalcount//10
  
    n = no_of_teams
   
    initial_remaing_players = toalcount % 10
...
def download(request, id):
    ...
    for i in range(n+1):
        ...
        T0 = player = Player.objects.filter(
            tournament_city_id=city, status="success", group=groupid, team=i)
    ...
    if request.GET.get('Team_no'):
        t = request.GET['Team_no']
        dict = {}
        T0 =  Player.objects.filter(
            tournament_city_id=city, status="success", group=groupid, team=t)
        
        dict[t] = T0

        Heading = cityName+"-"+groupid+"-Team-"+t
    li=[]
    for T in T0:
    ...
    return render(request, "download.html", {"id": id, "cityName": cityName, "dict": dict, "n": n, "Heading": Heading, 'range': range(n+1),"remain2":remain2,"groupid":groupid})
...
def formteam(request, id):
    ...
    #         try:
    #             player_obj = Player.objects.filter(team=i, position1=p, tournament_city=city)
    #             for alpha in player_obj:
                   
    #                 teams.at[i, p] =+ alpha.ikfuniqueid
    #                 print("print")
    #         except  Exception as e:
    #             print(e)
                
    #             teams.at[i, p] = "NOT Assigned"
    # print(teams,"vnsjkfhsd")
   
   

    return redirect('team_table2', id=id_)

    # return render(request, "formteam2.html", {"id": id, "cityName": cityName, "player": player, "remain2": remain2,"remain":remain,"teams":teams,"count":count })
