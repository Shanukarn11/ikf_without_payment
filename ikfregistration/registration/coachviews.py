...
def admitcard(request):
    ...
    if request.method=='POST':
        try:
            ...
            lang="en"
            langqueryset = MasterCoachLabels.objects.filter(keydata__in=['timer_text','coach_pass_message']).values('keydata', lang)
            for item in langqueryset:
...
def coachplayer(request):
    if(request.method=="GET"):
        ...
        try:
            ...
            playermodel.pic_file=playermodel.ikfuniqueid+'.png'
            # playermodel.document_id_file=playermodel.ikfuniqueid+'.png'
            playermodel.save(update_fields=['ikfuniqueid','playeruploadid','whoisfilling','pic_file'])
        ...
        try:
            ...
            pic_url='media/images/'+str(playermodel.pic_file)
            # doc_url='media/documents/'+str(playermodel.document_id_file)
            playermodel.delete()
            if pic_url !="":
                os.remove(pic_url)
            # if doc_url !="":
